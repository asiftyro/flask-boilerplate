...
class UpdatePostForm(FlaskForm):
    ...
    slug = StringField('Slug', validators=[DataRequired(),
                                           Regexp(regex=r'^[a-zA-Z0-9\-]+$', message='Must contain only Alphanumeric and Hyphen characters'),
                                           Length(max=128)])
    post_position = SelectField("Position", coerce=int)
    visible = SelectField("Visibile",  coerce=int)
...
class CreatePostForm(FlaskForm):
    ...
    slug = StringField('Slug', validators=[DataRequired(), Regexp(
        regex=r'^[a-zA-Z0-9\-]+$', message='Must contain only Alphanumeric and Hyphen characters'), Length(max=128)])
    post = TextAreaField('Post Content', validators=[DataRequired()])
